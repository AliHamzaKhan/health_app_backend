...
from app.schema.app_schemas import CREATE_DATA_PROCESS_SCHEMA, INSERT_DATA_PROCESS_SCHEMA, FIND_DATA_PROCESS_SCHEMA
...
class DataProcessService:
    ...
    async def save_data_process(self, data_process: DataProcessRequest) -> str:
        ...
        try:
            await self.database.execute(CREATE_DATA_PROCESS_SCHEMA)
            await self.database.execute(INSERT_DATA_PROCESS_SCHEMA, *data_to_insert)
    ...
    async def get_data_process(self, user_id: str, request_type: AiRequestType = None):
        query = FIND_DATA_PROCESS_SCHEMA
    ...
    async def get_single_data_process(self, data_process_id : str):
        ...
        #        """
        try:
            process_data = await self.database.fetch_one(FIND_DATA_PROCESS_SCHEMA, data_process_id)
            print(process_data)
